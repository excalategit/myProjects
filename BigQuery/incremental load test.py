...
def extract_transform():
    ...
    try:
        ...
        source_table = source_table[source_table['modified_date'] == datetime.today().date() - timedelta(days=3)]
        ...
        to_gbq(source_table, 'my-dw-project-01.bq_upload.stg_bq_project1', project_id=project_id, if_exists='append')
    ...
    try:
        ...
        USING (
            SELECT * EXCEPT(row_num) FROM (
                ...
                FROM `my-dw-project-01.bq_upload.stg_bq_project1`
                WHERE modified_date = (select max(modified_date) from `my-dw-project-01.bq_upload.stg_bq_project1`)
            ) WHERE row_num = 1) s
